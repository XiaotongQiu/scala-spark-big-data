...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    val question = postings.filter(_.postingType.equals(1))
      .map(p => (p.id, p))
    val ans = postings.filter(_.postingType.equals(2))
...
//      .filter(_._1 != -1)
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    scored.map(r => (firstLangInTag(r._1.tags, langs).getOrElse(-1) * langSpread, r._2))
      .filter(_._1 >= 0).cache()
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val pairs = vectors.map(v => (findClosest(v, means), v))
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = pairs.groupByKey()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val langsCnt = vs.groupBy(_._1).map(r => (r._1, r._2.size))
//      langsCnt.foreach(println)
      val mostCommonLang = langsCnt.maxBy(_._2)
      ...
      val sz = scores.size
      val medianScore: Int    = sz % 2 match
        case 0 => (scores(sz / 2 - 1) + scores(sz / 2)) / 2
        case _ => scores(sz / 2)
